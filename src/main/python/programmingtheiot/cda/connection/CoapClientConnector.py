...
class CoapClientConnector(IRequestResponseClient):
	...
	def __init__(self):
		...
		logging.info("Configuration data "+str(self.config))
...
# 	 a callback to handle the response to the discovery request
...
#This will issue a discovery request to the server, which will provide
# a list of resource names (fully qualified) that are currently registered with the server.	
	def sendDiscoveryRequest(self, timeout: int = IRequestResponseClient.DEFAULT_TIMEOUT) -> bool:
		...
		response = self.coapClient.discover(timeout=timeout)
		if response:
			logging.debug(response.pretty_print())
			self.pathList = response.payload.split(",")
			index = 0
			for path in self.pathList:
				for char in "<\>":
			...
			logging.info("No response received")
# 			self.coapClient.get(path = '/.well-known/core', callback = self._onDiscoveryResponse(response), timeout = timeout)
		


# 	def sendDeleteRequest(self, resource: ResourceNameEnum, timeout: int = IRequestResponseClient.DEFAULT_TIMEOUT) -> bool:
# 		logging.debug("Testing GET with path:"+ resource.value)
# 		response = self.coapClient.delete(resource.value)
# 		if response:
# 			logging.debug(response.pretty_print())
# 		else:
...
# 		if not resource.value and self.pathList:
# 			for pathEntry in self.pathList:
# 				logging.debug("Issuing with path entry:" + str(pathEntry))
# 				response = self.coapClient.get(pathEntry)
# 			if response:
# 				logging.debug(response.pretty_print())
# 			else:
# 				logging.debug("No response received")
# 		elif resource.value:
# 			logging.debug("Testing GET with path:"+ resource.value)
# 			response = self.coapClient.get(resource.value)
# 			if response:
# 				logging.debug(response.pretty_print())
# 			else:
# 				logging.debug("No response received")
# 		else:
# 			logging.warning("Cant test GET")
			""" 	 get functionality to CoapClientConnector"""
	...
	""" handle responses on getting GET requests		"""			
...
# 		if resource:
# 			logging.debug("Issuing PUT with path: " + resource.value+", payload "+payload)
# 			response = self.coapClient.post(resource.value, payload)
# 			if response:
# 				logging.debug(response.pretty_print())
# 			else:
# 				logging.debug("No response received")
	"""POST functionality for Coap Client Connector"""
	...
	def _initClient(self):
		...
		logging.info("Created coap client: "+str(self.coapClient))
